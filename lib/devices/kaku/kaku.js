...
module.exports = RFDevice => class Kaku extends DefaultDevice(RFDevice) {
	...
	static validateData(data) {
		...
		let errorMessages = {}
		if (data) {
			if (data.address.length !== 26) { error = true; errorMessages.address = 'Wrong address size'; }
			...
			if (typeof data.group === 'undefined') { error = true; errorMessages.unit = 'Group undefined'; }
			if (typeof data.state === 'undefined') { error = true; errorMessages.unit = 'State undefined'; }
		}
		if (error) {
			console.log(errorMessages);
			return false;
		} else {
			console.log('Kaku validated in dataToPayload');
			return true;
		}
	}
};
